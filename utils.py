...
def visualize_image_and_graph(img, nodes, edges, viz_img_size=512, kp_points=None, kp_labels=None):
    # img is rgb
    ...
    # Draw sampled keypoints if provided
    if kp_points is not None and kp_labels is not None:
        # Convert to the same format as nodes
        normalized_kps = kp_points / viz_img_size
        
        for i, (point, label) in enumerate(zip(normalized_kps, kp_labels)):
            x, y = point * viz_img_size
            # Green for key points (label=1), Red for non-key points (label=0)
