...
# Trace all node indices along a segment, stop at crossover/intersection/dead end.
...
# will get two segments with the same start and end points but different lengths, so the two segments can be merged into a non-isolating loop.
...
# This function merges nodes that are close to each other using the DBSCAN clustering algorithm,
# calculates the center coordinates of each cluster as new nodes, and then updates the edge information based on the new nodes.
...
# This function checks the nearby nodes of each edge, if the node distance is less than the given threshold,
# then splits the edge at the node into two new edges (not fully understood).
...
def find_intersection(segment1, segment2):
    ...
    # if the intersection point is not an endpoint, it is a valid point, return
...
# Build an R-tree index to accelerate the search for segment intersection points,
...
# The subdivision process is implemented by inserting new points and generating new edges, and finally a new igraph object is built and returned.
