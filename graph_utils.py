...
# trace a segment on the graph, return the list of node indices, stop when encountering crossover/intersection/dead end
# note that this algorithm does not consider non-isolated loops, if there are non-isolated loops, the returned segment is a line that is not connected at the beginning and end
...
# we will get two segments with the same start and end points but different lengths, so we can merge the two segments into a non-isolating loop.
...
def normalize_segments(coords, segments):
 ...
 # this function resamples each polyline to have the same number of points
...
# this function merges nodes that are close to each other using DBSCAN clustering,
# calculates the center coordinates of each cluster as new nodes, then updates the edge information based on the new nodes
...
# this function splits edges at nearby nodes, if the node distance to the edge is less than the given threshold,
# then the edge is split into two new edges at the node
...
def igraph_from_adj_dict(graph, coord_transform):
    ...
    edges = set([(min(src, tgt), max(src, tgt)) for src, tgt in edges]) # edges:[a->b, b->a] -> [min(a, b), max(a, b)]
...
# build an R-tree index to speed up line segment intersection finding,
# then traverse each line segment, find all intersection points
...
# subdivide each edge in the given graph into multiple segments, so that each segment has a length close to the specified resolution.
# the subdivision process is implemented by inserting new points and generating new edges, then building and returning a new igraph object
...
#         keep_nbr = np.greater(neighbor_scores, 1.0) # a list only contains True or False
